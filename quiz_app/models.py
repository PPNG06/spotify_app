...
class User(models.Model):
    values = [x for x in genres]
    values_name = [x.capitalize() for x in genres]
    STYLE_CHOICES_L = [('-','-')]
    for x in range(0,len(genres)):
        STYLE_CHOICES_L.append((values[x],values_name[x]))
    STYLE_CHOICES = tuple(STYLE_CHOICES_L)
    ENERGY = ((0.8,'Turn Up'),(0.5,'Chill'),(0.3,'Study'))
    name = models.CharField(max_length=100, default=" ")
    style1 = models.CharField(max_length=100,choices = STYLE_CHOICES, default=STYLE_CHOICES[0])
    style2 = models.CharField(max_length=100,choices = STYLE_CHOICES, default=STYLE_CHOICES[0])
    style3 = models.CharField(max_length=100,choices = STYLE_CHOICES, default=STYLE_CHOICES[0])
    artist = models.CharField(max_length=1000, default=" ")
    mood = models.FloatField(max_length=1000,choices = ENERGY, default=ENERGY[0])
    linked_track = models.CharField(max_length=1000, default=" ")

    def __str__(self):
        return self.name+' (style: '+self.style1+', mood: '+self.mood+')'
